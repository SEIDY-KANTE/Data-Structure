...
class BST{
        ...
        int FindIndex(const Object& item, int currentIndex=0){
            while(true){
                if(IndexFill[currentIndex]==0 || currentIndex==SIZE)
                    return -1;
                else if(IndexFill[currentIndex]==1 && items[currentIndex]==item)
                    return currentIndex;
                else if(item<items[currentIndex]) //left add
                    currentIndex=2*currentIndex+1;
                else if(item>items[currentIndex]) //right add
	...
	public:
        ...
        void inorder(int index=0){
			if(index<SIZE && IndexFill[index]==1){
				inorder(2*index+1);
				cout<<items[index]<<" ";
				inorder(2*index+2);
			}
